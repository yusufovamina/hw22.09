...
//var dbtrs = debtors.Where(x => x.FullName[0] == 'A' || x.FullName[0] == 'O' || x.FullName[0] == 'I' || x.FullName[0] == 'E' || x.FullName[0] == 'U').ToList();
//dbtrs.ForEach(Console.WriteLine);


//  2. Вывести тех должников, имеющих почту в домене rhyta.com или dayrep.com
//var debtors1 = debtors.Where(debtor => debtor.Email.Contains("rhyta.com") || debtor.Email.Contains("dayrep.com")).ToList();
//debtors1.ForEach(Console.WriteLine);

//  3. Вывести тех должников в возрасте от 26 до 36 лет.
//var debtors2=debtors.Where(x=> 2023-x.BirthDay.Year>=26 && 2023-x.BirthDay.Year  <= 36).ToList();
//debtors2.ForEach(Console.WriteLine);

//  4. Вывести тех должников, сумма долга которых не превышает 5000.
//var debtors3 = debtors.Where(x => x.Debt < 5000).ToList();
//debtors3.ForEach(Console.WriteLine);


//  5. Вывести тех должников, полное имя которых длиннее 18 символов, и номер телефона которых содержит 2 или больше семерок.
//var debtors4 = debtors.Where(x => x.FullName.Length>18 && x.Phone.Count(x => x == '7') >= 2).ToList();
...
//var debtors5=debtors.Where(x=> (2023 - x.BirthDay.Year) % 2 == 0)
//    .OrderBy(x=> x.BirthDay.Year.ToString().Sum(c => c - '0')).ToList();
//debtors5.ForEach(Console.WriteLine);

//  7. Вывести тех должников, родившихся зимой, отсортировав по почтовому индексу (5 цифр в конце адреса).
// var debtors6=debtors.Where(debtor => debtor.BirthDay.Month == 12 || debtor.BirthDay.Month <= 2)
//    .OrderBy(debtor => debtor.Address.Substring(debtor.Address.Length - 5)).ToList();
...
//var AverageDebt = debtors.Average(debtor => debtor.Debt);
//var debtors7 = debtors
...
//var debtors8 = debtors
//            .Where(debtor => !debtor.Phone.Contains("8")).ToList();
//foreach (var debtor in debtors8)
//{
//    Console.WriteLine($"{debtor.FullName.Split(". ")[1]}  {2023-debtor.BirthDay.Year}  {debtor.Debt}");
//}
